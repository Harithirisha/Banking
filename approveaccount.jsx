...
export const sendEmail = async (toEmail, templateId) => {
  try {
    const templateParams = {
      to_email: toEmail,
      firestore_api_link: 'https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/customerAccounts',
...
function ApproveAccountsPage() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/customerAccounts');
  ...
  const updateRequestStatus = async (requestId, newStatus, email, phoneNumber) => {
    try {
      ...
      await axios.patch(`https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/customerAccounts/${requestId}`, {
      ...
      if (newStatus === 'approved') {
        ...
        await axios.post('https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/Account', {
          fields: {
            accountNumber: { integerValue: accountNumber },
