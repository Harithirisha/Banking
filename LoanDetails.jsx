...
function LoanDetails() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/Loan');
    ...
    try {
      if (status === 'approved') {
        ...
        if (sanctionedAmount !== null) {
          const accountsResponse = await axios.get('https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/customerAccounts');
          const accounts = accountsResponse.data.documents;
 
          const accountResponse = await axios.get('https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/Account');
          ...
          if (accountToUpdate) {
            ...
            await axios.patch(`https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/customerAccounts/${accountToUpdate.name.split('/').pop()}`, {
              fields: {
            ...
            await axios.patch(`https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/Account/${accToUpdate.name.split('/').pop()}`, {
              fields: {
      ...
      await axios.patch(`https://firestore.googleapis.com/v1/projects/hkb-bank/databases/(default)/documents/Loan/${id}`, {
        fields: {
